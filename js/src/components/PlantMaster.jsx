...
import { Search, Plus, Edit2, Trash2, X, Save, ArrowLeft, ChevronLeft, ChevronRight, Factory } from "lucide-react";

const PlantMaster = ({ onClose }) => {
  ...
  const API_BASE =
    "https://ktfrancesrv2.kalyanicorp.com/api/v1/collection/cost_plant_master";
  ...
  const fetchPlantList = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to fetch Plant records");

      const data = await response.json();
      setPlantList(data.objects || []);
      showNotification("Plant records loaded successfully", "success");
    } catch (error) {
      console.error("Fetch failed:", error);
      showNotification("Failed to load Plant records: " + error.message, "error");
    } finally {
      setLoading(false);
    }
  };

  // Create Plant record (POST)
  const createPlant = async () => {
    ...
    try {
      ...
      showNotification("Plant record created successfully", "success");
      setShowModal(false);
      fetchPlantList();
      resetForm();
  ...
  // Update Plant record - Using PUT method with ID in URL
  const updatePlant = async () => {
    ...
    try {
      ...
      showNotification("Plant record updated successfully", "success");
      setShowModal(false);
  ...
  const deletePlant = async (plantId, plantName) => {
    ...
    try {
      ...
      showNotification("Plant record deleted successfully", "success");
      fetchPlantList();
    } catch (error) {
  ...
  const totalPages = Math.ceil(filteredPlantList.length / itemsPerPage);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredPlantList.slice(indexOfFirstItem, indexOfLastItem);
  ...
  // Get badge color based on plant type
  const getPlantTypeBadgeColor = (type) => {
    const colors = {
      Forging: { bg: "#fee2e2", color: "#dc2626" },
      Machining: { bg: "#dbeafe", color: "#1e40af" },
      "Heat Treatment": { bg: "#fed7aa", color: "#c2410c" },
      Assembly: { bg: "#dcfce7", color: "#166534" },
      Finishing: { bg: "#fef3c7", color: "#d97706" },
      "Quality Control": { bg: "#ede9fe", color: "#7c3aed" },
        ...
        >
          ...
          >
            ...
            >
              ...
              <input
                type="text"
                placeholder="Search plant records..."
                value={searchTerm}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {currentItems.map((plant) => {
                      const badgeColor = getPlantTypeBadgeColor(plant.plant_type);
                      return (
                        <tr
              ...
              {totalPages > 1 && (
                ...
                >
                  <div style={{ fontSize: "0.8125rem", color: "#64748b" }}>
                    ...
                    {Math.min(indexOfLastItem, filteredPlantList.length)} of{" "}
                    {filteredPlantList.length} records
                  </div>
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("short_name", e.target.value.toUpperCase())
                    }
                    placeholder="e.g., MDWA"
                    style={{
                      width: "100%",
                      padding: "0.625rem 0.875rem",
                      background: "#f8fafc",
