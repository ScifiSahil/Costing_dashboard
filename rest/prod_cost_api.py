...
class ProdCost(JsonAPI):
    """Simple Internal API to read cost/ton views (day / month / year)."""
    pass


@Internal.mount(app=ProdCost, path="prod_cost")
def _mount_app():
    return ProdCost()
...
class ProdCostHandler:
    """
    Handler logic:
    - ?view=day   -> day table
    - ?view=month -> month table with unified filter
    - ?view=year  -> year table
    """
    ...
    def get_data(self, request):
        ...
        if view == "month":
            ...
            # final SQL with ORDER BY (sorting)
            sql = (
                "SELECT * FROM %s "
                "WHERE YEAR_NO = %d "
                "AND MONTH_NO BETWEEN %d AND %d "
                "ORDER BY YEAR_NO, MONTH_NO, PLANT, COST_HEAD"
        ...
        for row in rs:
            ...
            # ---------------------------------------------------
            # TEMP FALLBACK: Static COST_PER_TON (remove later)
            # ---------------------------------------------------
            if record.get("cost_per_ton") in (None, 0):
                try:
                    record["cost_per_ton"] = record["total_amount"] / 1000
                except:
                    record["cost_per_ton"] = None
            # ---------------------------------------------------
...
@ProdCost.path(model=ProdCostHandler, path="")
def _path():
    return ProdCostHandler()


@ProdCost.json(model=ProdCostHandler, request_method="GET")
