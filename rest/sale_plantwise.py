...
class SalePlantwise(JsonAPI):
    """Sale Cost Per Ton - EACH PLANT (day / month / year)."""
    pass


@Internal.mount(app=SalePlantwise, path="sale_plantwise")
def _mount_app():
    return SalePlantwise()
...
class SalePlantwiseHandler:
    """
    Sale Cost Per Ton API
    ...
    def get_data(self, request):
        try:
            ...
            # -------------------------------
            # Month logic (same as production)
            # -------------------------------
            if view == "month":
...
@SalePlantwise.path(model=SalePlantwiseHandler, path="")
def _path():
    return SalePlantwiseHandler()


@SalePlantwise.json(model=SalePlantwiseHandler, request_method="GET")
